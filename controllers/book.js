...
exports.createBook = (req, res, next) => {
  try {
    ...
    // Suppression de l'image téléchargée en cas d'échec d'ajout de livre
    if (req.file && fs.existsSync(req.file.path)) {
      fs.unlinkSync(req.file.path);
    }
    return res
      .status(400)
      .json({ message: "Tous les champs doivent être valide" });
  }
};
...
exports.modifyBook = (req, res, next) => {
  try {
    ...
    // Suppression de l'image téléchargée en cas d'échec de modification du livre
    if (req.file && fs.existsSync(req.file.path)) {
      fs.unlinkSync(req.file.path);
    }
    return res
